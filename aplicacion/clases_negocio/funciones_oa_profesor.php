...
function insertar_comentario($contenido, $idusuario, $id_objeto_aprendizaje, $Imagen, $Fecha) {
    $conexion = new Conexion();
    $statement = 'INSERT INTO comentario (contenido,idusuario,id_objeto_aprendizaje,Imagen,Fecha) VALUES (?, ?, ?,?,?)';
    $consulta = $conexion->prepare($statement);
    if ($consulta->execute(array($contenido, $idusuario, $id_objeto_aprendizaje,$Imagen,$Fecha))) {
        return true;
    } else {
        return false;
    }
...
//inicio funciones de comentario
function insertar_Imagen($imagen, $idusuario, $id_objeto_aprendizaje) {
    $conexion = new Conexion();
    $statement = 'INSERT INTO comentario (contenido,idusuario,id_objeto_aprendizaje) VALUES (?, ?, ?)';
    $consulta = $conexion->prepare($statement);
    if ($consulta->execute(array($imagen, $idusuario, $id_objeto_aprendizaje))) {
        return true;
    } else {
        return false;
    }
